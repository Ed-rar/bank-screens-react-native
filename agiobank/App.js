...
const ValidateLogin = async (cpf, senha, setActivity, navigation, setModalVisible, setTextModal) => {
  ...
  //Tela Inicial
  else if(cpf.trim() === CPF && senha.trim() === SENHA){
    setActivity(true);
    setTimeout(function() {
    setActivity(false);   return navigation.navigate('Home');
    }, 2000);
  }
...
function TelaLogin({ navigation }) {
  ...
  return (
    <View style={Estilos.container}>
      ...
      <View style={Estilos.button}>      
        <Button title="Login" color onPress={() => {
          ...
          if(ValidateLogin(user, password, setActivity, navigation, setModalVisible, setTextModal) === true ){    navigation.navigate('Home')}}
          }/>
      </View>
      ...
      <View>
        <Text style={{fontWeight: 'bold', marginTop: 20, textAlign: 'center'}}>Ainda nao tem uma conta?</Text>

        <Text style={Estilos.link} onPress={() =>{ {navigation.navigate('Cadastro')}}}>Cadastre-se</Text>
      </View>
    </View>
  )
}
  ...
  return (
    <NavigationContainer>
      <Stack.Navigator initialRouteName="Login" screenOptions={{
        ...
        <Stack.Screen name="Login" component={TelaLogin} options={{ title: 'AgioBank - Login'  }} />

        <Stack.Screen name="Home" component={TelaInicial} options={{title: 'AgioBank - Tela Incial'  }} />

        <Stack.Screen name="Cadastro" component={TelaDeCadastro} options={{ title: 'AgioBank - Tela de Cadastro'  }} />
      </Stack.Navigator>
    </NavigationContainer>
  );
}
